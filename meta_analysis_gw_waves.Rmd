...
table(total_results_waves$mean_year, useNA = "always")

```

```{r, results='hide', warning=FALSE}
# Estimate non-linear effects by including time as dummies
total_results_waves <- fastDummies::dummy_cols(total_results_waves, select_columns = "mean_year")

model_step_2_att_dum <- rma(yi = mu_time,
              sei = mu_time_sd,
              data = total_results_waves,
              mods = ~ mean_year_2021 + attitude_cat + ambiguous + data,
              method = "REML",
              test = "knha")

model_step_2_att_dum
...
```{r}
# Also non-linear mean year for variance
...
kable(total_meta, digits = 3, caption = "Appendix X. Meta-regression on climate change attitudes and polarization per wave",
  ...
  add_header_above(c(" " = 2, "Time-only model" = 3, "Indepent var model" = 3, "Interaction model" = 3)) %>%
   footnote(
    ...
    save_kable("./output/appendix/meta_regression_table_per_wave_NOVEMBER.html")
...
#And the interaction model
model_3_mean_df <- data.frame(matrix(ncol = 5, nrow = 12))
colnames(model_3_mean_df) <- c("Mu/Sigma", "Variable", "Estimate", "SE", "p")
model_3_mean_df$Variable <- c("Intercept", "Mean year (centered around mean)", "Attitude affective (ref = behavioral)", "Attitude cognitive", "Ambiguous (ref = no)", "Dataset ESS (ref = EB)", "EVS", "I&O", "ISSP", "LISS", "Motivaction", "SOCON")
model_3_mean_df$`Mu/Sigma` <- c("Mean attitudes", "", "", "", "", "", "", "", "", "", "", "")
...
#And the interaction model
model_3_var_df <- data.frame(matrix(ncol = 5, nrow = 12))
colnames(model_3_var_df) <- c("Mu/Sigma", "Variable", "Estimate", "SE", "p")
model_3_var_df$Variable <- c("Intercept", "Mean year (centered around mean)", "Attitude affective (ref = behavioral)", "Attitude cognitive", "Ambiguous (ref = no)", "Dataset ESS (ref = EB)", "EVS", "I&O", "ISSP", "LISS", "Motivaction", "SOCON")
model_3_var_df$`Mu/Sigma` <- c("Polarization in attitudes", "", "", "", "", "", "", "", "", "", "", "")
...
kable(total_meta, digits = 3, caption = "Appendix X. Meta-regression on climate change attitudes and polarization per wave",
  ...
  add_header_above(c(" " = 2, "Time-only model" = 3, "Indepent var model" = 3, "Interaction model" = 3)) %>%
   footnote(
    ...
    save_kable("./output/appendix/meta_regression_table_per_wave_NOVEMBER_WITH_DATASETS.html")
```
