...
```{r}
#And for ess2023
load("./data/all_waves/ess23sel.RData")

sex <- c('Male', 'Female')
sex <- c(0.50, 0.50)

attributes(ess23sel$age)
ess23sel$age_cat[ess23sel$age >=15 & ess23sel$age <= 30] <- 1
ess23sel$age_cat[ess23sel$age >=31 & ess23sel$age <= 45] <- 2
ess23sel$age_cat[ess23sel$age >=46 & ess23sel$age <= 60] <- 3
ess23sel$age_cat[ess23sel$age >=61] <- 4
...
ess23sel$caseid <- 1:length(ess23sel$sex)

# To use anesrakefinder, the variables in the original dataset have to be numeric
ess23sel$sex <- as.numeric(ess23sel$sex)
ess23sel$urban <- as.numeric(ess23sel$urban)
anesrakefinder(targets, ess23sel, choosemethod = "total")

outsave <- anesrake(targets, ess23sel, caseid = ess23sel$caseid,
  verbose= FALSE, cap = 5, choosemethod = "total",
  type = "pctlim", pctlim = .05 , nlim = 5,
  iterate = TRUE , force1 = TRUE)
summary(outsave)

ess23sel$weightvec  <- unlist(outsave[1])
table(ess23sel$weightvec)
...
# Also add the weights for 2017
load("./data/all_waves/evs17sel.Rdata")

sex <- c('Male', 'Female')
sex <- c(0.50, 0.50)

attributes(evs17sel$age)
evs17sel$age_cat[evs17sel$age >=15 & evs17sel$age <= 30] <- 1
evs17sel$age_cat[evs17sel$age >=31 & evs17sel$age <= 45] <- 2
evs17sel$age_cat[evs17sel$age >=46 & evs17sel$age <= 60] <- 3
evs17sel$age_cat[evs17sel$age >=61] <- 4

age_cat <- c('u30', '3145', '4661', '61o')
age_cat <- c(0.19, 0.28, 0.22, 0.23)
...
# To use anesrakefinder, the variables in the original dataset have to be numeric
evs17sel$sex <- as.numeric(evs17sel$sex) + 1 # For anesrake, everything has to start with 1
evs17sel$age_cat <- as.numeric(evs17sel$age_cat) 

evs17sel$caseid <- 1:length(evs17sel$sex)

anesrakefinder(targets, evs17sel, choosemethod = "total")

outsave <- anesrake(targets, evs17sel, caseid = evs17sel$caseid,
  verbose= FALSE, cap = 5, choosemethod = "total",
  type = "pctlim", pctlim = .05 , nlim = 5,
  iterate = TRUE , force1 = TRUE)
summary(outsave)

evs17sel$weightvec  <- unlist(outsave[1])
save(evs17sel, file = "./data/all_waves/evs17sel.Rdata")

load("./data/final_data/evssel.Rdata")
evssel <- dplyr::left_join(evssel, evs17sel)
...
## Motivaction {-}
```{r}
load("./data/all_waves/mot2019.Rdata")
