...
color_scale <- c ("#E02023", "#1676E3", "#0AC760", "#1ABC9C", "#85C1E9" , "#EB984E", "#AF7AC5" , "#F1C40F")
...
datayear$color[datayear$dataset == "LISS"] <- "#0AC760"
...
yearplot <- ggplot(data = datayear, aes(x = year, y = dataset, color = color)) +
  ...
  labs(x = "Year",
       y = "Dataset") +
  geom_point(data = datayear, aes(x = year, y = dataset), color = datayear$color, size = 2) +
  geom_point(data = first_last_year, x = first_last_year$first_year, y = datayear$dataset) + 
  geom_point(data = first_last_year, x = first_last_year$last_year, y = datayear$dataset) + 
...
kable(model_new, digits = 3, caption = "Table 3. All pooled effects mean attitudes and polarization", 
  ...
  add_header_above(c(" " = 2, "Time-only model" = 3, "Indepent var model" = 3, "Interaction model" = 3)) %>%
   footnote(
    ...
    save_kable("./output/pooled_effects_table_NOVEMBER.html")
```
...
kable(total_df, digits = 3, caption = "Appendix D. Pooled effects mean attitudes and polarization age, education and gender.", 
 ...
 save_kable("./output/appendix/pooled_effects_edugenage_NOVEMBER.html")
